...
func (g *generator) renderStaticWebsite(providerSchema *tfjson.ProviderSchema) error {
	...
	err = filepath.WalkDir(g.websiteTmpDir, func(path string, d os.DirEntry, err error) error {
		...
		// skip special top-level generic resource, data source, and function templates
		...
		case "resources/":
			...
			metadataFilePath := filepath.Join(g.ProviderExamplesDir(), "resources", resName, "metadata")

			if resSchema != nil {
				tmpl := resourceTemplate(tmplData)
				render, err := tmpl.Render(g.providerDir, resName, g.providerName, g.renderedProviderName, "Resource", exampleFilePath, importFilePath, metadataFilePath, resSchema)
				if err != nil {
